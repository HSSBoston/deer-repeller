...
def getSunsetToday(response):
    return getSunsetForecast(response, 0)

def getSunsetTomorrow(response):
    return getSunsetForecast(response, 1)

def getSunsetForecast(response, daysLater):
    if response == None:
        return None
    if daysLater > 5:
        return None
    if response.status_code == 200:
        ...
        if "daily" in responseDict:
            forecast = responseDict["daily"][daysLater]
            sunsetUnixTime = forecast["sunset"]
            sunsetDt = datetime.fromtimestamp(sunsetUnixTime)
            return sunsetDt
        else:
            return None
    else:
...
def getSunriseToday(response):
    return getSunriseForecast(response, 0)

def getSunriseTomorrow(response):
    return getSunriseForecast(response, 1)

def getSunriseForecast(response, daysLater):
    if response == None:
        return None
    if daysLater > 5:
        return None
    if response.status_code == 200:
        ...
        if "daily" in responseDict:
            forecast = responseDict["daily"][daysLater]
            sunriseUnixTime = forecast["sunrise"]
            sunriseDt = datetime.fromtimestamp(sunriseUnixTime)
            return sunriseDt
        else:
            return None
    else:
